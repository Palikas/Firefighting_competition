...
# Start continuous rotation at 300 degrees/second
...
def exec_8(api):
    #ked uvidi sviecku, chod ku nej
    api.LMM.run_time(500, 1800)
    api.LMM.run_time(-500, 1800)
    #api.RMM.run_time(-1000, 8000)
    dialkovy = UltrasonicSensor(PORT.DS)
    colorn = ColorSensor(PORT.LCS)
    while dialkovy.distance() > 500:
        api.straight(50, 200)
        if dialkovy.distance() < 500:
            dialka = dialkovy.distance() / 1.33333333
            api.turn(90, -100)
